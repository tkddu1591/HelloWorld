...
function UserPopup({
                       popupSetting, setCommentsList, setCommentReply, communityNo, commentType, setRefreshTrigger
                   }: UserPopupProps, {}) {
    ...
    function handleClickAxios(item) {
        ...
        axios.post(API_BASE_URL + item.action, {
            commentNo: popupSetting.axiosData, communityNo: communityNo, commentType: commentType
        })
            .then((res) => {
        ...
        <div
            ...
            {popupSetting.condition.map((item, index) => {
                ...
                return (
                    <span
                        key={item.data}
                        onClick={() => item?.type === 'axios' && popupSetting.axiosData ? handleClickAxios(item) :
                            handleClick(item)}
                        style={{
                            paddingLeft:  '5px',
                            cursor:       'pointer',
