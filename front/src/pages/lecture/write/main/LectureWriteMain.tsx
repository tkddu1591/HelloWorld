...
function LectureWriteMain() {
    ...
    if (lecture?.tagList) console.log(lecture?.tagList);
    ...
    console.log(tags);
    return (
        <Container style={{marginTop: '100px'}} className={'lectureMainWrite'}>
            <Row>
                            ...
                            <Button
                                ...
                                onClick={async () => {
                                    if (window.confirm('정말 삭제하시겠습니까?')) {
                                        await apiClient.delete(`/lecture?lectureNo=${lectureNo}`);
                                        await navigate('/my/info');
                                    }
                                }}>
                                삭제
                            </Button>}
            ...
            <Row style={{marginTop: '20px'}}>
                ...
                <Col className={'lectureTitle'} md={2}>
                    <div>
                        <h5>
                            <span className={'red'}>*</span> 가격
                        </h5>
                        <Input
                            type="text"
                            placeholder="가격 입력"
                            value={lecture?.price ? lecture.price.toLocaleString() : ''}
                            onChange={e => {
                ...
                <Col className={'lectureTitle'} md={2}>
                    <div>
                        <h5>
                            할인율 <span style={{fontSize: '14px'}}>( % )</span>
                        </h5>
                        <Input
                            type="number"
                            placeholder="할인율 입력"
            ...
            <Row style={{marginTop: '30px'}}>
                <Col md={6}>
                    <div>
                        <h5>
                            <span className={'red'}>*</span> 태그
                        </h5>
                        <SelectBox
                            options={tagSort.list}
                            isMulti={tagSort.isMulti}
                            setSelect={setLecture}
                            selectName={'tagList'}
                            value={selectTag}
                            placeholder={tagSort.placeholder}
                            isSearchable={tagSort.isSearchable}></SelectBox>
                    </div>
                </Col>
                <Col md={6}>
                    <div>
                        <h5>
                            <span className={'red'}>*</span> 레벨
                        </h5>
                        <SelectBox
                            options={levelSort.list}
                            setSelect={setLecture}
                            selectName={'levelNo'}
            ...
            <Row style={{marginTop: '30px'}}>
                <Col md={12}>
                    <FileUpload
                        ...
                        fileName={'thumbName'}
                        fileURL={'thumbURL'}
                        value={lecture?.thumbName}
                        setFileName={setLecture}
                        setFileURL={setLecture}
                ...
                <Col style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                    ...
                    <Button
                        ...
                        onClick={async () => {
                            ...
                            await apiClient
                                .post(`/lecture/write/main`, lecture)
                                .then(res => {
                                    if (res.status === 200 && typeof newLectureNo === 'number') {
                                        if (lectureNo === null) navigate('/lecture/write/content?lectureNo=' + res.data);
                                        else navigate('/lecture/write/content?lectureNo=' + lectureNo);
                                    }
                                })
                                .catch(err => {
